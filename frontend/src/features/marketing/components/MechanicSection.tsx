...
export function MechanicSection() {
  ...
  const handleApplyClick = () => {
    if (!user) {
      authGateStore.open({ action: 'apply as a mechanic', role: UserRoles.PROVIDER });
    } else if (hasRole(user, 'provider')) {
      navigate({ to: ROUTES.PROVIDER_DASHBOARD });
    } else {
      info('You are logged in as a vehicle owner. Please sign out to create a provider account.');
    }
  };
